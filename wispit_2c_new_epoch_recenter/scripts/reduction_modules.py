...
class CircleMaskModule(ProcessingModule):
    ...
    def run(self) -> None:
        ...
        if self.m_both_gaps:
            # inner
            ellipse_outer_inner = draw.ellipse(50, 50, 13, 13, rotation=np.radians(1.5 + rot_shift))
            mask[ellipse_outer_inner] = 0
            ellipse_inner_inner = draw.ellipse(50, 50, 5, 5, rotation=np.radians(2 + rot_shift))
            mask[ellipse_inner_inner] = 1
